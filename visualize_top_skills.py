...
def show_top_skills(df_jobs, df_skills, df_skills_job):
    job_title_options = df_jobs['job_title_sort'].dropna().unique()
    selected_job_title = st.selectbox("Pilih Job Title Sort:", options=sorted(job_title_options))

    job_ids = df_jobs.loc[df_jobs['job_title_sort'] == selected_job_title, 'job_id'].unique()
